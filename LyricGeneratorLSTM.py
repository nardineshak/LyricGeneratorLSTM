...
df = pd.read_csv("/Users/nardineshak/Downloads/lyrics.csv", sep="\t")
df.head()
pdf = pd.read_csv('/Users/nardineshak/Downloads/PoetryFoundationData.csv',quotechar='"')
pdf.head()

# cleansing the data 
# only taking the first four verses of the song
# process every song in order to split each verse 
# and join the resulting text in a single text
def split_text(x):
   text = str(x['lyrics'])
#    print(text)
   sections = text.split('\\n\\n')
...
# creating a dict of words -> index & index -> word 
...
valid_seqs = []
end_seq_words = []
# going through substrings of size MIN_SEQ
# if the substring doesn't have an uncommon word add it 
# to valid_seqs and end_seqs_words
...
# takes in list of sentences and their corresponding next words, and batch size
#The function uses these inputs to generate batches of sequences (x) 
# and their corresponding next words (y) continuously.
